...
class Team:
    def __init__(self, roster: List[ae.Animal], equipment: List[ae.Equipment]):
        self.animals = roster
    ...
    def size(self):
        ...
        for animal in self.animals:
            if not isinstance(animal, ae.Empty):
                i += 1
        return i
    ...
    def faint(self, target):
        self.equipment[target] = ae.Unarmed()
        self.animals[target] = ae.Empty()
    ...
    def summon(self, animal, position):
        ...
        self.equipment.insert(ae.Unarmed(), position)
    ...
    def leftmost_unit(self):
        i = 0
        while isinstance(self.animals[i], ae.Empty):
    ...
    def friend_ahead(self):
        for j in range(self.acting, len(self.animals)):
            if isinstance(self.animals[j], ae.Empty):
    ...
    def random_friend(self):
        ...
        a.remove(self.acting)
        for i in a:
            if isinstance(self.animals[i], ae.Empty):
                a.remove(i)
        if a == []:
            return None
