...
class Tier3:
    ...
    def ox(agent: 'MessageAgent'):
        agent.event_raising_animal.permanent_buff(agent.event_raising_animal.level, 0)
    ...
    def sheep(agent: 'MessageAgent'):
        unit = Ram()
        unit.hp = 2 * agent.event_raising_animal.level
        unit.atk = 2 * agent.event_raising_animal.level
    ...
    def turtle(agent: 'MessageAgent'):
        units: List[Animal] = agent.team.friends_behind(agent.event_raising_animal.level)
        for unit in units:
            unit.held = Melon()
