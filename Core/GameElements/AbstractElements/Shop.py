import Core.GameElements.Objects.Base
...
class Shop:
    ...
    def clear_unfrozen(self):
        for slot in self.roster:
            if not slot.is_frozen:
                slot.item = Core.GameElements.Empty

        j = 0
        for i in range(self._shop_size):
            if isinstance(self.roster[i].item, Core.GameElements.Empty):
                self.roster[j].item = self.roster[i].item
                j += 1
        # if the first item of the food shop is an animal send it back!!
        if isinstance(self.roster[self._shop_size].item, Core.GameElements.Animal):
    ...
    def fill_shop(self):
        ...
        """
        for slot in self.roster:
            if slot.item is Core.GameElements.Empty:
...
class ShopSlot:
    ...
    def __init__(self, mode):
        self.mode = mode
        self.spawner = Core.GameElements.Spawner(mode)
        self.item = Core.GameElements.Empty()
...
class AnimalShopSlot(ShopSlot):
    ...
    def __init__(self, item: Core.GameElements.Animal):
